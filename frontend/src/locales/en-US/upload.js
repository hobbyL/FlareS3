export default {
  upload: {
    hint5gb: 'Multi-file queue upload, up to 5GB per file',
    hint100mb: 'Multi-file queue upload, up to 100MB per file (server-mediated)',
    expiresIn: 'Expiration',
    uploadConfig: 'Upload config',
    uploadDir: 'Directory',
    downloadPermission: 'Download access',
    requireLogin: 'Login required',
    publicDownload: 'Public',
    uploading: 'Uploading: {filename}',
    remaining: '{time} remaining',
    fileExpire: 'File will be deleted after {days} day(s)',
    fileNeverExpire: 'File never expires',
    shortLink: 'Short link',
    directLink: 'Direct link',
    copy: 'Copy',
    calculating: 'Calculating...',
    preparing: 'Preparing...',
    fileTooLarge: 'File exceeds 5GB limit',
    fileTooLargeServer: 'File exceeds {max}MB limit for this storage type',
    uploadFailed: 'Upload failed',
    uploadCanceled: 'Upload canceled',
    uploadSuccess: 'Upload succeeded!',
    shortLinkCopied: 'Short link copied',
    directLinkCopied: 'Full link copied',
    loadUploadOptionsFailed: 'Failed to load upload options',
    expireNever: 'Never expires',
    expireDays: '{days} day(s)',
    seconds: '{value}s',
    minutesSeconds: '{minutes}m {seconds}s',
    hoursMinutes: '{hours}h {minutes}m',
    secondsOnly: '{value}s',
    minutesOnly: '{value}m',
    hoursOnly: '{value}h',
    queue: {
      title: 'Upload queue',
      status: {
        queued: 'Queued',
        uploading: 'Uploading',
        success: 'Uploaded',
        error: 'Failed',
        cancelled: 'Cancelled',
      },
      actions: {
        cancel: 'Cancel',
        retry: 'Retry',
        remove: 'Remove',
      },
    },
    errors: {
      partMissingEtag: 'Part {partNumber} did not return an ETag',
      incompleteMultipart: 'Incomplete multipart upload: {uploaded}/{total}',
    },
  },
  uiUpload: {
    dropHint: 'Click or drag a file to upload',
    dropDesc: 'Multi-file queue upload',
    brutalDrag: 'Drag file here',
    brutalOrClick: 'or click to upload',
  },
}
